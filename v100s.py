...
def get_batch_size(patch_size):
    ...
    if patch_size <= 256: return 40     # 当前建议
    return 24
...
def get_patch_size(epoch):
    ...
    if epoch <= 150:  return 256     # ← 延长 256 的训练时间
    return 320
...
def main(rank, world_size):
    ...
    cur_batch = get_batch_size(cur_ps) // world_size   # 每卡 batch
    train_loader, train_sampler = make_ddp_loader(
        train_ds, cur_batch, NUM_WORKERS)
    # val / test 只在 rank-0 跑，不需要 DDP sampler
    ...
    # DDP 包装；find_unused_parameters=False 更快
    ...
    ema = EMA(model, decay=0.9999)   # EMA 跟 raw model（不是 DDP wrapper）
    ...
    # criterion = CompositeLoss(lc=1.0, ls=1.2, lp=0.05, lf=0.1, lw=0.3, ln=0.4, le=0.15).to(device)
    # 180轮后可以改lr建议
    ...
    for epoch in range(start_epoch, NUM_EPOCHS + 1):
        ...
        for bi, (ldct, ndct) in enumerate(train_loader):
            ...
            # EMA 只在 rank-0 更新（shadow 不参与梯度同步）
            if is_main:
                ema.update(model)
        ...
        # 跨卡汇总 loss（可选，用于 history 记录准确）
        loss_tensor = torch.tensor(total / len(train_loader),
                                   device=device)
        ...
        if is_main and epoch % 10 == 0:
            ...
            avg_p, avg_s = evaluate(
                ...
                SAVE_DIR, tile=256, overlap=64, tag=f'val_ep{epoch}')
            model_ddp.train()

            save_data = {
                'epoch':     epoch,
                'best_psnr': best_psnr,
                'model':     model.state_dict(),   # raw model，不带 module. 前缀
...
if __name__ == '__main__':
    world_size = torch.cuda.device_count()   # 自动检测，双卡=2
    assert world_size >= 2, f"需要至少 2 张 GPU，检测到 {world_size} 张"
    print(f"检测到 {world_size} 张 GPU，启动 DDP 训练")

    # torchrun 会自动注入 RANK / LOCAL_RANK / WORLD_SIZE 环境变量
    # 直接调用 main，rank 从环境变量读取
